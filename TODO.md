...
- OG image generation at `/_og/s/*` — `@takumi-rs/core` + `@takumi-rs/wasm` are already installed, `nitro.json` excludes the path, just needs a server route handler
- the three CV-page action buttons (download pdf, linkedin, email for refs) are stubs with no handlers
- mobile breakpoints — the source design ships no `@media` queries; some headings (notably the h1 at 196px on `?v=unhinged`) overflow narrow viewports
