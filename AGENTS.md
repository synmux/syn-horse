...
bun run clean                 # Remove build artefacts and caches
...
bun run db:sql                # Run a SQL query
bun run db:drop               # Drop a named table
bun run db:drop-all           # Drop all tables
bun run db:squash             # Consolidate migrations into one file
bun run db:mark-as-migrated   # Mark migration as applied without running
...
## Experimental Features (19 active)
...
| `wrangler.json`             | Cloudflare Worker config (symlink → `.output/server/wrangler.json`) | No — generated by `bun run build`        |
