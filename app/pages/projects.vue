<script setup lang="ts">
  ...
  const cards = computed<ProjectCard[]>(() => {
    ...
    return [...live, ...dead].sort(
      (first, second) => Number(second.yr) - Number(first.yr)
    );
  });
  ...
  <div class="page-shell">
    ...
    <TransitionGroup tag="div" name="grave" class="proj-grid">
      ...
      >
        ...
        <div v-if="card.abandoned" class="proj-epitaph">
          † {{ card.epitaph }}
        </div>
        <div v-else class="proj-url">↗ {{ card.url }}</div>
        <div class="proj-tags">
          <span v-for="(t, i) in card.tags" :key="i" :class="['tg', t.k]"
            >{{ t.l }}</span
          >
        </div>
      </div>
