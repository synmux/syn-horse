<script setup lang="ts">
  ...
  const allCommands = computed<Command[]>(() =>
    commandsEnabled.value ? [...KONAMI_COMMANDS, ...COMMANDS] : COMMANDS
  );
    ...
    >
      <div class="flex items-center gap-2.5 border-b border-void-4 px-4 py-3.5">
        ...
        <input
          ...
          spellcheck="false"
        >
        <span class="font-mono text-[10px] tracking-[0.14em] text-paper-3"
          >{{ filtered.length }}
          results</span
        >
      </div>
      <div ref="listEl" class="max-h-[50vh] overflow-y-auto">
        ...
        >
          <span class="font-mono text-[11px] tracking-[0.14em] text-paper-3"
            >—</span
          >
          <div>
            <div class="font-sans text-[14px] text-paper">nothing matches.</div>
            <div class="font-mono text-[11px] tracking-[0.04em] text-paper-3">
              try less.
            </div>
          </div>
          <span />
      ...
      >
        ...
        <span
          >{{ SITE.name }}
          ·
          <a :href="commitUrl" class="hover:text-cool"
            >{{ commitHash }}</a
          ></span
        >
      </div>
