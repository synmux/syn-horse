...
The user requested generation of a SKILL.md best practices section for vue-router v5.0.6 containing exactly 13 best practice items formatted as structured markdown. Each item must include a description and reasoning, plus a source link with section anchor or line reference. Maximum output is 213 lines total. Inline code blocks should appear in approximately 1 in 4 items (not more). The output file must be written to `/Users/dave/src/github.com/synmux/syn-horse/.claude/skills/vue-router-skilld/.skilld/_BEST_PRACTICES.md`. Verification must be completed via `skilld validate` command.
