...
export default {
  async fetch(
    ...
    const analyticsPath = pathname.startsWith("/")
...
async function handleRedirect(
  ...
  const redirectPath: string = pathname.startsWith("/")
    ? pathname.substring(1)
    : pathname;

  // Get the list of valid redirects
  const { redirects: validRedirects, cacheHit } = await getValidRedirects(
    env,
    ctx
  );
 ...
 */
async function getValidRedirects(
  env: Env,
  ctx: ExecutionContext
): Promise<ValidRedirectsResult> {
  try {
