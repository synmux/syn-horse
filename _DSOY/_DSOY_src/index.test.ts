...
function createMockEnv(assetMap: AssetMap = {}) {
  ...
  return {
    env,
    kvStore: store,
    assetsFetch: env.ASSETS.fetch as ReturnType<typeof createAssetsFetch>,
    writeDataPoint,
  };
}
...
describe("worker fetch", () => {
  ...
  it("serves the root page from assets", async () => {
    const { env, assetsFetch, writeDataPoint } = createMockEnv({
      "/": { body: "home" },
    });
    const response = await handler.fetch(
      new Request("https://dcw.soy/"),
      env,
  ...
  it("serves static assets directly when present", async () => {
    const { env, assetsFetch, writeDataPoint } = createMockEnv({
      "/soy.webp": { status: 200, body: "soy" },
    });
    const response = await handler.fetch(
      new Request("https://dcw.soy/soy.webp"),
      env,
      createExecutionContext()
    );
  ...
  it("serves the not-found page for invalid redirects", async () => {
    const { env, kvStore, assetsFetch, writeDataPoint } = createMockEnv({
      "/not-found.html": {
        body: "not-found",
        headers: { Location: "/should-remove" },
      },
    });
    kvStore.set(KV_KEY, {
      redirects: ["existing"],
    ...
    const response = await handler.fetch(
      new Request("https://dcw.soy/missing"),
      env,
      createExecutionContext()
    );
  ...
  it("fetches redirects remotely on cache miss and caches the result", async () => {
    ...
    const cached = (await env.KV.get(KV_KEY, "json")) as {
      redirects?: string[];
    } | null;
...
describe("analytics data model", () => {
  ...
  it("reports redirect_fallback with cache miss when redirect list is unavailable", async () => {
    ...
    const response = await handler.fetch(
      new Request("https://dcw.soy/anything"),
      env,
      createExecutionContext()
    );
...
describe("stats API", () => {
  ...
  it("serves /stats from ASSETS (not intercepted by API handler)", async () => {
    ...
    const response = await handler.fetch(
      new Request("https://dcw.soy/stats"),
      env,
      createExecutionContext()
    );
