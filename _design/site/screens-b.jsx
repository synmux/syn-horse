...
function Blog({ openPost }) {
  ...
  const filtered = useMemoB(
    () =>
      filter === "all"
        ? window.POSTS
        : window.POSTS.filter((p) => p.tags.includes(filter)),
    [filter]
  );
  return (
    <div className="container blog">
      <div className="eyebrow">
        ▶ /blog · {window.POSTS.length} posts · infrequent · lowercase
      </div>
      <h1>
      ...
      <p className="lede">
        ...
        occasionally my body. infrequent. unedited. no editor will ever fix
        that.
      </p>
      <div className="filter">
        <span
          className={"tg " + (filter === "all" ? "on" : "")}
      ...
      <div className="feed-line">
        rss feed: <a href="/feed.xml">/feed.xml</a> · subscribe with the reader
  ...
  return (
    <div className="container post">
      ...
      <div className="meta">
        ...
        {!p.real && (
          <span style={{ color: "var(--pop)" }}>· FROM THE FUTURE</span>
        )}
      </div>
      <div className="body">
        <p>
        ...
        <h2>the setup</h2>
        <p>
          here's the thesis. i had a problem, i refused to leave the house, and
          i had a half-charged thinkpad. you can guess what happened next.
        </p>
        <p>
          i'd been running this exact stack for about six months —{" "}
          <code>nuxt 4</code>, <code>cloudflare workers</code>,{" "}
          <code>@nuxt/content</code>, <code>tailwind</code>,{" "}
          <code>daisyui</code> — and the thing that finally broke me was the{" "}
          <b>build pipeline</b>. specifically, the part where it lies about
...
i lost the file. i'm not sorry.`}</code>
        ...
        <h2>what changed</h2>
        <p>
          the fix, as is tradition, was three lines of yaml. but the journey
          there involved <a href="#">prometheus</a>, <a href="#">grafana</a>, a
          misconfigured tailnet, two pots of coffee, and a brief out-of-body
          experience.
        </p>
        <blockquote>"the kernel panics, but romantically."</blockquote>
        <p>
          that's an error message i would die for. instead i get "
          <i>something went wrong, our team has been notified</i>." nothing went
          wrong. somebody wrote a bad query and capitalism happened. say so.
        </p>
        <h2>what to do</h2>
        <ul>
        ...
        <p>
          that's it. that's the post. there's no call to action. there's no "if
          you liked this, consider subscribing." you got here. you read it.
          that's the contract. thanks.
        </p>
      </div>
      <div className="post-foot">
        ...
        <span>
          <a
            href="https://github.com/synmux/syn-horse"
            target="_blank"
            rel="noopener"
          >
            edit on github →
      ...
      <div className="role">
        ...
        <div className="right">
          <p>
            ...
            names + contact details on request — i'm not putting them on a
            public page.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="container contact">
      ...
      <div className="grid">
        ...
        <div>
          ...
          <div className="ask">
            ...
            <br />i don't list phone numbers in plaintext (scrapers gonna
            scrape). email me first, mention which messenger, and i'll send you
            a number. signal preferred.
          </div>
        </div>
      </div>
      ...
      <div className="footer-note" style={{ marginTop: 64 }}>
        <span>
          ◆ usually responds within 48h · sometimes 2 weeks · rarely never
        </span>
        <span>
          do not pitch me web3, "ai-driven" anything, or recruitment for crypto
        </span>
      </div>
    </div>
  );
...
function Domains() {
  return (
    <div className="container domains">
      ...
      <p className="lede">
        i collect tlds the way other people collect houseplants. one is the main
        site. the rest redirect, park, or quietly ask a question about ducks.
      </p>
        ...
        {window.DOMAINS.map((d) => (
          <div key={d.tld} className={"dom-row " + d.cls}>
            ...
            <div className="status">
              {d.status === "ok" && (
                <span className="ok">● {d.statusText}</span>
              )}
              {d.status === "warn" && (
                <span className="warn">● {d.statusText}</span>
              )}
              {d.status === "park" && (
                <span className="park">○ {d.statusText}</span>
              )}
            </div>
          </div>
        ))}
...
function NotFound({ go }) {
  return (
    <div className="notfound">
      ...
      <div className="yell">
        this page <span className="accent">does not exist</span>. never did. go
        home.
      </div>
      <div className="console">
        ...
        <div className="mu">
          ...
          <span className="danger">no such file or directory</span>
        </div>
        <div>
          <span className="pr">syn@horse</span> <span className="mu">~/$</span>{" "}
          dmesg | tail
        ...
        <div className="mu">
          → kernel: page fault at 0x{Math.random().toString(16).slice(2, 10)} —
          request was nonsense
        </div>
        <div className="mu">
          → kernel: i checked. it's not here. it never was.
        </div>
        <div className="mu">
