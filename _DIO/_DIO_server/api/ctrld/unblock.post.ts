...
export default defineEventHandler(async (event) => {
  const { domain, auth, profile, permanent } =
    await readBody<ctrldUnblockRequest>(event);
  ...
  try {
    const response = await unblockDomain(
      { domain, profileId, permanent },
      useRuntimeConfig(event).ctrldApiKey
    );
    return ok(event, {
    ...
    return error(
      event,
      {},
      `Failed to create override: ${err instanceof Error ? err.message : "Unknown error"}`,
      500
    );
  }
});
