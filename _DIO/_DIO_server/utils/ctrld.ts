...
function normaliseDomain(input: string): string {
  ...
  if (!domain) {
    logger.warn("Falling back to best-effort host", {
      input,
      normalised: host,
    });
  }
...
export async function unblockDomain(request: UnblockRequest, apiKey: string) {
  ...
  const previousDeleted = await ensureRuleDeleted(
    normalisedDomain,
    request.profileId,
    apiKey
  );
...
async function ensureRuleDeleted(
  ...
  return (
    (await $fetch(
      `https://api.controld.com/profiles/${profileId}/rules/${normalisedDomain}`,
      {
        method: "DELETE",
        headers: {
          Accept: "application/json",
