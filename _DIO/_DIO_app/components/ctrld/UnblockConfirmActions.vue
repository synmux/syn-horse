  ...
  <div>
    <div class="card-actions justify-end mt-4 space-x-2">
      <button
        ...
        @click="handleTempUnblock"
      >
        <span
          v-if="isSubmitting && !isSubmittingPermanent"
    ...
    <dialog :open="modalOpen" class="modal" @close="closeModal">
      <div class="modal-box max-w-xl">
        ...
        <p class="py-2">
          Think before you click! Is this block protecting you?
        </p>
        <p class="text-sm opacity-70">
          Permanent unblocking will disable protections for this domain on your
          profile FOREVER. Dave can remove the override, but
        ...
        <div class="modal-action">
          ...
          >
            ...
            ></span>
            <span v-else>
              Unblock Permanently<span v-if="countdown > 0">
                ({{ countdown }})</span
              >
            </span>
          </button>

          <button class="btn" :disabled="isSubmitting" @click="closeModal">
            Cancel
          </button>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  async function performUnblock(permanent: boolean) {
    try {
      ...
      if (error.value) {
        throw new Error(
          error.value.statusMessage || "An unknown error occurred."
        );
      }

      emit(
        "success",
        `Successfully unblocked ${props.domain}${permanent ? " permanently" : " for 15 minutes"}.`
      );
