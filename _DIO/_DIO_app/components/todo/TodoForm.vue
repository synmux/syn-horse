<template>
  <div class="space-y-4">
    ...
    <div
      v-if="status.message"
      :class="['alert', status.type === 'success' ? 'alert-success' : 'alert-error']"
    >
      <Icon
      ...
      <div class="card-body">
        ...
        <form class="space-y-4" @submit.prevent="onSubmit">
          ...
          <fieldset class="fiqweldset">
            ...
            <label class="label" for="description">
              ...
              <NuxtLink to="https://www.markdownguide.org/cheat-sheet">
                Markdown
                <Icon
                  name="pepicons-pop-question-circle-filled"
                  class="text-red-400"
                />
              </NuxtLink>
              here.
          ...
          <p class="text-xs text-subtext0">
            This form posts to
            <code class="font-['Victor_Mono']">/api/todo</code>. The backend
            route is stubbed and will accept data via POST.
          </p>
        </form>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  async function onSubmit() {
    ...
    try {
      ...
      const msg =
        res.message || "Submitted (stub). I'll wire this to Linear shortly.";
      status.type = "success";
      status.message = msg;
